...
import com.salesianostriana.dam.cuadromandointegral.controladores.RegistrarseController;
...
@WebMvcTest(controllers = RegistrarseController.class)
@MockBean(JpaMetamodelMappingContext.class)
class TestRegistrarseController {
	...
	public void whenPostWithDataAndRoleSuperAdmin_thenReturns201() throws Exception {
		...
		CreateUserEntityDto createUserDto = CreateUserEntityDto.builder().username("Usuario1").password("1234")
				.password2("1234").email("usuario1@usuario.com").idColegio(1L).rol(Role.USER).build();
		...
		when(userEntityServicio.newUserEntity(createUserDto, createUserDto.getRol(), createUserDto.getIdColegio())).thenReturn(user);
		when(userEntityDtoConverter.convertUserEntityToUserEntityDto(user)).thenReturn(userDto);
		
			
		MvcResult result = mockMvc
								.perform(post("/registrarse").contentType("application/json;charset=UTF-8")
